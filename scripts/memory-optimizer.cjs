...
class MemoryOptimizer {
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    return config;
        ...
        if (!content.includes('webpackWorkerCount') && !content.includes('maxMemoryGenerations')) {
...
module.exports = nextConfig`
  ...
  async createOptimizedScripts() {
    try {
      ...
      const memoryScripts = {
        'dev:memory-optimized': 'node --max-old-space-size=4096 --max-semi-space-size=256 ./node_modules/.bin/next dev -p 9002',
        'dev:low-memory': 'node --max-old-space-size=2048 --max-semi-space-size=128 ./node_modules/.bin/next dev -p 9002',
        'build:optimized': 'node --max-old-space-size=8192 ./node_modules/.bin/next build',
  ...
  async createMemoryMonitor() {
    ...
    try {
      const monitorPath = path.join(this.projectRoot, 'scripts', 'memory-monitor.js');
