...
import { Award, Download, ImageIcon, Loader2 } from "lucide-react";
import Image from "next/image";
import { memo, useCallback, useMemo, useRef, useState } from "react";

// Type for serialized certificate passed from server component
type SerializedCertificate = Omit<Certificate, "issueDate"> & {
  issueDate: string;
};
...
function CertificateViewComponent({
  certificate,
  settings,
  ...
  const qrCodeUrl = useMemo(() => {
    if (certificate.certificateUrl && certificate.certificateUrl !== "#") {
      ...
      return `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
        verificationUrl
      )}&qzone=1&color=000000`;
    }
    return "";
  }, [certificate.certificateUrl]);

  const handleDownload = useCallback(
    async (formatType: "pdf" | "png") => {
  ...
  const dateFromTimestamp = useMemo(() => {
    // Handle serialized date string from server component
    if (typeof certificate.issueDate === "string") {
      return new Date(certificate.issueDate);
    }
  ...
  const formattedDate =
    dateFromTimestamp && isValid(dateFromTimestamp)
            ...
            <div className="w-full h-full border-[3px] border-[#facc15] bg-white p-[20px] flex flex-col justify-between relative">
              ...
              <div className="w-full grid grid-cols-2 z-10">
                <div className="text-left">
                  ...
                  <p className="font-sans font-bold text-[14px] text-gray-800 tracking-wider mt-1">
              ...
              <div className="text-center z-10 py-2">
                <p className="text-[14px] font-sans font-semibold tracking-[0.2em] text-gray-500">
                  DRIVING SCHOOL
                </p>
                <h1 className="text-[36px] font-bold tracking-[0.1em] text-[#1e3a8a] mt-1">
                  CERTIFICATE
                </h1>
                <p className="text-[14px] font-sans font-semibold tracking-[0.2em] text-gray-500 mt-1">
                  OF COMPLETION
                </p>
              </div>
              ...
              <div className="text-center z-10 py-2">
                ...
                <p className="text-[14px] text-gray-700 max-w-2xl mx-auto">
                  ...
                  <span className="font-bold">
                    {certificate.course} ({certificate.type})
                  </span>{" "}
                  course with {settings.schoolName}.
                </p>
              </div>
              ...
              <div className="w-full flex flex-col items-center z-10 pt-4">
                <div className="w-full flex items-end justify-between mt-2">
                  <div className="text-center w-1/3">
                    <p className="text-[20px] font-cursive text-gray-800 h-[30px]">
                      {directorName}
                    </p>
                    <hr className="border-gray-600 my-1 w-4/5 mx-auto" />
                    <p className="text-[10px] font-sans font-semibold text-gray-600 tracking-wider">
                      DIRECTOR
                    </p>
                  </div>
                  ...
                  <div className="text-center w-1/3 flex flex-col items-center justify-end h-full">
                    ...
                    <p className="text-[9px] text-gray-600 font-sans mt-1">
                      Verify at: {verificationDomain}
                    </p>
                  </div>

                  <div className="text-center w-1/3">
                    <p className="text-[14px] font-semibold text-gray-800 h-[30px]">
                      {formattedDate}
                    </p>
                    <hr className="border-gray-600 my-1 w-4/5 mx-auto" />
                    <p className="text-[10px] font-sans font-semibold text-gray-600 tracking-wider">
