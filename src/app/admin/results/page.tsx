...
import { Trash2, Edit, Eye, Sparkles, Lightbulb, Loader2, ThumbsUp, TrendingUp, ClipboardList, PlusCircle, User } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { getResults, deleteResult, type TestResult } from "@/services/resultsService";
...
export default function AdminResultsPage() {
  ...
  const fetchResults = async () => {
    ...
    try {
        ...
        toast({
            ...
            description: "Could not load test results from the database."
        });
    } finally {
        setIsLoading(false);
    }
  ...
  const handleGetFeedback = async (rawResults: string) => {
    ...
    setFeedback(null);
    try {
      const aiResult = await getPersonalizedFeedback({ testResults: rawResults });
              ...
              {isLoading ? (
                Array.from({length: 5}).map((_, i) => (
                    <TableRow key={i}>
                        ...
                        <TableCell className="text-right"><div className="flex justify-end gap-2"><Skeleton className="h-8 w-8" /><Skeleton className="h-8 w-8" /><Skeleton className="h-8 w-8" /></div></TableCell>
                    </TableRow>
                ))
              ) : results.length > 0 ? (
                results.map((result) => (
                    ...
                    <TableCell className="flex items-center gap-4">
                        <Avatar>
                        <AvatarImage src={(result as any).studentAvatar} alt={result.studentName} data-ai-hint="person portrait" />
                        <AvatarFallback style={{ backgroundColor: generateAvatarColor(result.studentName) }}>
                    ...
                    <TableCell>{format(result.date.toDate(), "PPP")}</TableCell>
                    ...
                    <TableCell className="text-right space-x-2">
                        <Dialog onOpenChange={(isOpen) => !isOpen && setFeedback(null)}>
                            ...
                            <DialogContent className="sm:max-w-[625px]">
                                <DialogHeader>
                                    <DialogTitle>Test Details for {result.studentName}</DialogTitle>
                                    <DialogDescription>
                                        Review raw results and generate AI-powered feedback.
                                    </DialogDescription>
                                </DialogHeader>
                                <div className="space-y-4 py-4">
                                <Alert>
                                    <AlertTitle>Raw Test Results</AlertTitle>
                                    <AlertDescription>{result.rawResults}</AlertDescription>
                                </Alert>

                                <Button onClick={() => handleGetFeedback(result.rawResults)} disabled={isAiLoading}>
                ...
                <TableRow>
                    <TableCell colSpan={6} className="h-24 text-center">No test results found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
