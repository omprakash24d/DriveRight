...
import { getInstructors, deleteInstructor, type Instructor } from "@/services/instructorsService";
...
export default function AdminInstructorsPage() {
  ...
  const fetchInstructors = async () => {
    ...
    try {
        ...
        const displayInstructors = fetchedInstructors.map(i => ({
            ...i,
            specialties: i.specialties.split(',').map(s => s.trim()).filter(Boolean)
        }));
        setInstructors(displayInstructors);
    } catch (error) {
        toast({
            variant: "destructive",
            title: "Error",
              ...
              {isLoading ? (
                Array.from({ length: 3 }).map((_, index) => (
                    <TableRow key={index}>
                        ...
                        <TableCell><div className="flex gap-2"><Skeleton className="h-6 w-16" /><Skeleton className="h-6 w-20" /></div></TableCell>
