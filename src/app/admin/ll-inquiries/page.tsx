...
import { getLlInquiries, updateLlInquiry, type LlInquiry, type LlInquiryStatus } from "@/services/llInquiriesService";
import { sendLlStatusUpdateEmail } from "@/app/ll-exam-pass/_lib/email-service";
import { useToast } from "@/hooks/use-toast";
import { Skeleton } from "@/components/ui/skeleton";

export default function LlInquiriesPage() {
  const [inquiries, setInquiries] = useState<LlInquiry[]>([]);
  ...
  const fetchInquiries = async () => {
    setIsLoading(true);
    try {
        const fetchedInquiries = await getLlInquiries();
        setInquiries(fetchedInquiries);
    } catch (error) {
        toast({ variant: "destructive", title: "Error", description: "Could not fetch LL inquiries." });
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      ...filteredInquiries.map(i => [
        ...
        sanitizeForCsv(format(i.timestamp.toDate(), "yyyy-MM-dd HH:mm:ss")),
