...
import { getLicensePrintInquiries, updateLicensePrintInquiry, type LicensePrintInquiry, type LicensePrintInquiryStatus } from "@/services/licensePrintInquiriesService";
import { sendLicensePrintStatusUpdateEmail } from "@/app/license-print/_lib/email-service";
import { useToast } from "@/hooks/use-toast";
import { Skeleton } from "@/components/ui/skeleton";

export default function LicenseInquiriesPage() {
  const [inquiries, setInquiries] = useState<LicensePrintInquiry[]>([]);
  ...
  const fetchInquiries = async () => {
    setIsLoading(true);
    try {
        const fetchedInquiries = await getLicensePrintInquiries();
        setInquiries(fetchedInquiries);
    } catch (error) {
        toast({ variant: "destructive", title: "Error", description: "Could not fetch DL inquiries." });
    } finally {
        setIsLoading(false);
    }
  };
              ...
              {isLoading ? (
                ...
                inquiries.map((inquiry) => (
                  <TableRow key={inquiry.id}>
                    ...
                    <TableCell>{format(inquiry.timestamp.toDate(), "PPP p")}</TableCell>
                    <TableCell>
