...
import { BookCopy, Loader2, PlusCircle, Save, Trash2 } from "lucide-react";
      ...
      try {
        ...
        if (settings) {
          form.reset({
            ...
            whyChooseUsPoints: settings.whyChooseUsPoints.map((p) => ({
              ...p,
              id: nanoid(),
            })),
          });
  ...
  const onSubmit: SubmitHandler<SettingsFormValues> = async (data) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Your changes have been successfully saved. It may take a moment for them to apply.",
      });
    } catch (error) {
      ...
      toast({
        ...
        title: "Save Failed",
        description:
          "Could not save settings to the database. Check console for details.",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            If your site is missing content, use these buttons to populate it
            with defaults. This action will not overwrite existing data.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap gap-4">
          <Button
            onClick={() => handleSeed(seedDefaultCourses, "Courses")}
            variant="outline"
            disabled={isSeeding}
          >
            {isSeeding ? (
          ...
          <Button
            onClick={() => handleSeed(seedDefaultInstructors, "Instructors")}
            variant="outline"
            disabled={isSeeding}
          >
            {isSeeding ? (
          ...
          <Button
            onClick={() => handleSeed(seedDefaultStudents, "Students")}
            variant="outline"
            disabled={isSeeding}
          >
            {isSeeding ? (
          ...
          <Button
            onClick={() => handleSeed(seedDefaultTestimonials, "Testimonials")}
            variant="outline"
            disabled={isSeeding}
          >
            {isSeeding ? (
          ...
          <Button
            onClick={() =>
              handleSeed(seedDefaultTrainingServices, "Training Services")
            }
            variant="outline"
            disabled={isSeeding}
          >
          ...
          <Button
            onClick={() =>
              handleSeed(seedDefaultOnlineServices, "Online Services")
            }
            variant="outline"
            disabled={isSeeding}
          >
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <InputField
                    ...
                    description="Include country code without '+', e.g., 919876543210"
                    isRequired
                  />
                  <TextareaField
                    control={form.control}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Manage which email addresses have administrative access to
                      this panel.
                    </p>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <TextareaField
                    control={form.control}
                    name="aboutPageText1"
                    label="About Page - Paragraph 1"
                    rows={4}
                  />
                  <TextareaField
                    control={form.control}
                    name="aboutPageText2"
                    label="About Page - Paragraph 2"
                    rows={4}
                  />
                </CardContent>
              </Card>
                ...
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <CardDescription>
                        Edit the points that appear in the &apos;Why Choose
                        Us&apos; section.
                      </CardDescription>
                    </div>
                    <Button
                  ...
                  {whyChooseUsFields.map((field, index) => (
                    <Card key={field.id} className="p-4 bg-muted/30">
                      <div className="grid md:grid-cols-2 gap-4">
                        <InputField
                          control={form.control}
                          name={`whyChooseUsPoints.${index}.title`}
                          label="Title"
                          isRequired
                        />
                        <InputField
                          control={form.control}
                          name={`whyChooseUsPoints.${index}.icon`}
                      ...
                      <TextareaField
                        control={form.control}
                        name={`whyChooseUsPoints.${index}.description`}
                        label="Description"
                        isRequired
                      />
                      <Button
            ...
            <TabsContent value="sections">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Manage the titles and subtitles for various sections on the
                    homepage.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid md:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <InputField
                    control={form.control}
                    name="quickServicesTitle"
                    label="Driving Services: Title"
                  />
                  <TextareaField
                    control={form.control}
                    name="quickServicesSubtitle"
                    label="Driving Services: Subtitle"
                    rows={2}
                  />
                  <InputField
                    control={form.control}
                    name="onlineToolsTitle"
                    label="Online Tools: Title"
                  />
                  <TextareaField
                    control={form.control}
                    name="onlineToolsSubtitle"
                    label="Online Tools: Subtitle"
                    rows={2}
                  />
                  <InputField
                    control={form.control}
            ...
            <TabsContent value="developer">
              <Card>
                ...
                <CardContent className="space-y-6">
                  <InputField
                    control={form.control}
                    name="developerNoteTitle"
                    label="Note Title"
                    isRequired
                  />
                  <TextareaField
                    control={form.control}
                    name="developerNoteText"
                    label="Note Text"
                    rows={5}
                    isRequired
                  />
